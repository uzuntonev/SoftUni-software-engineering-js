import { get, post, put, del } from './fetchData.js';
...
const actions = {
    'load-catches': loadCatches,
    'add-catches': addCatch,
...
async function addCatch() {
    const data = Object.keys(html.formInputs)
        .reduce((acc, curr) => {
            const input = html.formInputs[curr]();
            console.log(input); 
            if (input.value === '') {
                alert(`${input.className} input is empty, Please fill all input fields!`);
...
async function deleteHandler(ev) {
